...
cov_frac = as.character(as.numeric(args[5]))
cov_level = as.character(as.numeric(args[6]))
...
new<-data.frame(t(chr1_betavals_granges_df[,5:dim(chr1_betavals_granges_df)[2]]),pheno[,-c(1)])
ncolu<-dim(new)[2]
names(new)[ncolu]<-"Dx"
...
write.table(chr1_lm_results2, file=args[3], sep = "\t", row.names=FALSE)
...
write.table(chr1_lm_results4, file=args[4], sep = "\t", row.names=FALSE,quote=F)
